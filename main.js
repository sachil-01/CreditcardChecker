...
const validateCred = array => {
  let checkedCard = [];
  let sum2 = 0;
  const isEven = (array.length-1) % 2 === 0;
  ...
  const isValid = sum2 % 10 === 0;
  return isValid
}

const findInvalidCards = nestArr => {
  const invalidCards = nestArr.filter(arr => {
...
const stringToArray = string => {
  ...
  for(let i =0; i < string.length; i++) {
    numberArr.push(parseInt(string[i]))
  }
  return numberArr;
}

console.log(idInvalidCardCompanies([invalid5]))
console.log(stringToArray('2221007732149780'))
